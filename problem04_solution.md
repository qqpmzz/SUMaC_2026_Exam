...
$$
\frac{p}{\gcd(k, p)} = p \implies \gcd(k, p) = 1
$$

### 결론

$$
\boxed{\text{Successful} \iff \gcd(k, p) = 1}
$$
...
- Crank k회: $B'^k(n) = (n + k \cdot k) \mod p = (n + k^2) \mod p$
- Toggle: 검정 → 빨강
- 결과: (빨강, n + k²)

**비교**: $(n + 1) \equiv (n + k^2) \pmod{p}$
...
- $k - 1 \equiv 0 \pmod{p}$ ⟹ k = 1
- $k + 1 \equiv 0 \pmod{p}$ ⟹ k = p - 1
...
$$
\boxed{\text{Successful AND Special} \iff k = 1 \text{ 또는 } k = p - 1}
$$
